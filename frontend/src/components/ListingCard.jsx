...
const ListingCard = ({ listing }) => {
  return (
   ...
   <div className="bg-white border border-light shadow-sm hover:shadow-md transition-shadow overflow-hidden rounded-xl w-full sm:w-[330px]">
      <Link to={`/listing/${listing._id}`}>
        ...
        <div className="p-3 flex flex-col gap-2 w-full">
          <p className="text-lg font-semibold truncate text-primary">{listing.title}</p>
          <div className="flex items-center gap-1">
            <MdLocationOn className="text-primary" size={18} />
            <p className="text-sm text-secondarys truncate">{listing.address}</p>
          </div>
